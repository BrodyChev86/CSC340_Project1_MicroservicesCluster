...
public class ServiceNodeHandler implements Runnable{
    ...
    public ServiceNodeHandler(Socket socket, DatagramSocket datagramSocket) {
        ...
        connectedNodes.compute(service, (key, list) -> {
            if (list == null) {
        ...
        System.out.println("[INFO] Node connected: " + service + " (id=" + nodeId + "), total " +
                connectedNodes.get(service).size() + " node(s) for service.");
